  ...
  return (
    <aside className="w-64 h-screen flex flex-col border-r shadow-md ">
      <div className="mb-2 flex h-20 items-end justify-start bg-blue-600 p-4 md:h-40 m-2 rounded-lg">
        <div className="w-32 text-white md:w-40 -ml-2">
          <div className={`${lusitana.className} flex flex-row items-center leading-none text-white`}>
            <img src="/PT Logo.png" alt="" width={60} />
      ...
      <nav className="flex-1 mt-4 space-y-1">
        {navItems.map((item) => (
          <Link key={item.path} href={item.path} className={`flex items-center space-x-3 px-4 py-3 rounded-lg mx-2 transition ${
      ...
      <button className="flex items-center space-x-3 px-4 py-3 mx-2 mb-4 text-gray-700 hover:bg-gray-100 rounded-lg w-full">
        <LogOut size={20} />
