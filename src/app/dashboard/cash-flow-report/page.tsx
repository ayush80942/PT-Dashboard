...
const CashFlowReportPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCinemas = async () => {
      try {
        const res = await fetch('http://portal.picturetime.in:8080/api/cinema/list');
  ...
  const handleDownload = async () => {
    ...
    try {
      const res = await fetch(
        `http://portal.picturetime.in:8080/api/report/download-excel?cinemaId=${cinemaId}&from=${fromDate}&to=${toDate}`
      );
